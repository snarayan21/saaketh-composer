        ...
        for idx, arg in enumerate(args):
            if isinstance(arg, torch.Tensor) \
                ...
                if idx != 2:
                    ...
                    size_ratio = arg.element_size() // self.compressed_dtype_bytes
                    if len(arg.shape) == 1:
                        # The slice we take of a 1-dimensional tensor will be contiguous in memory.
                        # No transpositions needed.
                        output_view = arg[:output_shape[0]//size_ratio].view(
                            self.compressed_dtype)
                        new_args.append(output_view)
                    elif len(arg.shape) == 2:
                        # Have to use two views to get the output tensor in the compressed dtype,
                        # in the right shape, and contiguous in memory.
                        output_view = arg[:output_shape[0]//size_ratio,:].view(
                            self.compressed_dtype
                            ).view(
                                output_shape
