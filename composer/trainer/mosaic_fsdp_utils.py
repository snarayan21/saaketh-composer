...
if version.parse(torch.__version__) > version.parse('2.2.1') and version.parse(
    ...
    from torch.distributed.fsdp.fully_sharded_data_parallel import (_annotate_modules_for_dynamo, _auto_wrap,
                                                                    ...
                                                                    _init_state_dict_state,
                                                                    _register_all_state_dict_hooks,
                                                                    _register_flat_param, _init_extension)
    from torch.distributed.fsdp.wrap import CustomPolicy, ModuleWrapPolicy, _Policy
    from torch.distributed.tensor.parallel.fsdp import DTensorExtensions
    ...
    def post_unflatten_transform_t2p2p0(
        self, tensor: torch.Tensor, param_extension: Any
    ) -> torch.Tensor:
        from torch.distributed.tensor.parallel._data_parallel_utils import _unflatten_tensor
        from torch.distributed.fsdp._common_utils import _set_fsdp_flattened
        print("real compute stream id:", torch.cuda.default_stream().stream_id)
        if self.compute_stream is not None:
            print("compute stream for this dtensor is not none, id:", self.compute_stream.stream_id)
        print("current stream id is:", self.device_handle.current_stream().stream_id)
        stream = self.compute_stream or self.device_handle.current_stream()
        print("after the checks, stream id finally is:", stream.stream_id)
    ...
    DTensorExtensions.all_gather_dtensor = all_gather_dtensor_t2p2p0
    DTensorExtensions.chunk_dtensor = chunk_dtensor_t2p2p0
    DTensorExtensions.post_unflatten_transform = post_unflatten_transform_t2p2p0
...
@no_type_check
def _share_state_and_init_handle_attrs_t2p3(
    root_state: '_FSDPState',
    root_module: nn.Module,
) -> None:
    """Shares state from ``root_state`` to other FSDP states.
